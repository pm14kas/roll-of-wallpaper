...
public:
    ...
    Formula(std::string s, Interval(*p) (Interval d))
	{
		this->s = s;
		this->p = p;
	}
...
public:
    ...
    KeyValue(std::string s, Interval d)
	{
		this->s = s;
		this->d = d;
	}
    ...
    KeyValue operator+(Interval other)
	{
...
public:
    ...
    static bool tryParse(std::string param, Interval &result)
	{
		try
		{
...
private:
    Interval calc(std::vector<KeyValue> varmap, std::string f) //varmap - список переменных, f - строка, которую необходимо распарсить (каждый раз разная при рекурсивном подходе)
	{
		...
		for (unsigned int i = 0; i < varmap.size(); i++)
		{
			if (f == varmap[i].s)
			{
				return varmap[i].d; //значение переменной
			}
		...
		if (f == ("pi"))
			return 3.14159265358979;//пи
		else if (f == ("-pi"))
			return -3.14159265358979;//пи
		if (f == ("e"))
			return exp(1);//число e
		else if (f == ("-e"))
			return -exp(1);//число e
		size_t index;
		for (unsigned int func = 0; func < functions().size(); func++) //парсинг функций и порядка скобок
		{
			...
			if ((index = f.find(name)) != std::string::npos)
			{
				...
				return calc(varmap, (f.substr(0, static_cast<int>(index)) + std::to_string(functions()[func].p(calc(varmap, g.substr(1, i - 1)))) + g.substr(i + 1))); // вычисляем функцию, вычисленное значение подставляем вместо неё, здесь это второй аргумент
			}
		}
		if ((index = f.find("(")) != std::string::npos) //парсинг скобок
		{
		...
		if ((index = f.find("+")) != std::string::npos)
		{
			...
			} //не забываем о scientific notation 
			else
			{
				return calc(varmap, f.substr(0, index)) + calc(varmap, f.substr(index + 1));
			}
		}
        if (((index = inverse(f).find("-")) != std::string::npos) && (index) && ((inverse(f)[index + 1] == ' ')||(inverse(f)[index + 1] == '-'))) //иначе 5-3-2 превращается в 5-(3-2)
		{
			...
			{
				f = f.substr(0, index - 1) + f.substr(index);//убираем минус, который использовали для отмены унарного минуса
			}
			else
			{
                /*if (f[index-1]==' ')
...
public:
    Interval calc(std::vector<KeyValue> varmap)
	{
